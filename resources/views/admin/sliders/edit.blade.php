...
<div class="main-content-inner">
    <div class="main-content-wrap">
        ...
        <div class="wg-box">
            <form class="form-new-product form-style-1" action="{{ route('admin.sliders.update', $slider->id) }}" method="POST"
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Title" name="title" tabindex="0"
                        value="{{ old('title', $slider->title) }}" aria-required="true" required="">
                    @error('title')
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Tagline" name="title_small" tabindex="0"
                        value="{{ old('title_small', $slider->title_small) }}" aria-required="true" required="">
                    @error('title_small')
                    <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </fieldset>
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Subtitle" name="subtitle" tabindex="0"
                        value="{{ old('subtitle', $slider->subtitle) }}" aria-required="true" required="">
                    @error('subtitle')
                    <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </fieldset>
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Button Text" name="button_text" tabindex="0"
                        value="{{ old('button_text', $slider->button_text) }}" aria-required="true" required="">
                    @error('button_text')
                    <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </fieldset>
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="url" placeholder="https://example.com" name="button_link" tabindex="0"
                        value="{{ old('button_link', $slider->button_link) }}" aria-required="true" required="">
                    @error('button_link')
                    <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                </fieldset>
                <fieldset>
                    ...
                    <div class="upload-image flex-grow">
                        <div class="item" id="imgpreview" @if(!$slider->image) style="display:none" @endif>
                            <img src="{{ $slider->image ? asset('storage/' . $slider->image) : '' }}" class="effect8" alt="">
                        </div>
